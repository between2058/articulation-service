...
from models.schemas import ArticulationPart, ArticulationJoint, ArticulationData
...
class PhysicsInjector:
    ...
    def create_revolute_joint(
        ...
        logger.info(
            f"Created RevoluteJoint: {joint_path} ({parent_path} -> {child_path})"
        )
        return joint_path
    ...
    def create_prismatic_joint(
        ...
        logger.info(
            f"Created PrismaticJoint: {joint_path} ({parent_path} -> {child_path})"
        )
        return joint_path
    ...
    def create_fixed_joint(
        ...
        logger.info(
            f"Created FixedJoint: {joint_path} ({parent_path} -> {child_path})"
        )
        return joint_path
    ...
    def inject_physics(
        ...
        for part in articulation_data.parts:
            ...
            if stage.GetPrimAtPath(mesh_path).IsValid():
                self.apply_collision(
                    stage, mesh_path, collision_type=part.collision_type
                )
        ...
        logger.info(
            f"Injected physics for {len(articulation_data.parts)} parts "
            f"and {len(articulation_data.joints)} joints"
        )
    ...
    def _find_base_part(
        self, parts: List[ArticulationPart]
    ) -> Optional[ArticulationPart]:
    ...
    def _axis_to_token(self, axis: Tuple[float, float, float]) -> str:
        """
        Convert axis vector to USD axis token ("X", "Y", or "Z").
        """
    ...
    def _apply_joint_drive(
        ...
        logger.info(
            f"Applied DriveAPI ({drive_name}) to {joint_prim.GetPath()}: "
